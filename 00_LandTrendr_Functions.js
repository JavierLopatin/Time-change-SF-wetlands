...
// Description: A funciton to create a Landsat 5,7,8 medoid homogenization composition,  and
//              A function to apply the LandTrendr algorithm over a selected vegetation index or band
...
exports.ltCollector = function(run_param, aoi){
  ...
  */
  ...
  // ###########################################################################################################
  // ### END OF ltCollector FUNCITON ##########
  // ###########################################################################################################


  return annualSRcollection;
};


// Function to produce LandTrebdr segmentations with different indices and bands
exports.applyLT = function(VI, annualSRcollection, run_params, distDir){
  ...
  // apply the function to calculate the segmentation index and adjust the values by the distDir parameter - flip index so that a vegetation loss is associated with a postive delta in spectral value
   var ltCollection = annualSRcollection.map(NDVI)                                                   // map the function over every image in the collection - returns a 1-band annual image collection of the spectral index
                                         ...
                                         .set('system:time_start', img.get('system:time_start'))}); // ...set the output system:time_start metadata to the input image time_start otherwise it is null
  if(VI == "EVI"){
    var ltCollection = annualSRcollection.map(EVI)
                                         .map(function(img) {return img.multiply(distDir)
                                         .set('system:time_start', img.get('system:time_start'))});
  }
  if(VI == "GNDVI"){
    var ltCollection = annualSRcollection.map(GNDVI)
                                         .map(function(img) {return img.multiply(distDir)
                                         .set('system:time_start', img.get('system:time_start'))});
  }
  if(VI == "LSWI"){
    var ltCollection = annualSRcollection.map(LSWI)
                                         .map(function(img) {return img.multiply(distDir)
                                         .set('system:time_start', img.get('system:time_start'))});
  }
  if(VI == "TCB"){
    var ltCollection = annualSRcollection.map(TCB)
                                         .map(function(img) {return img.multiply(distDir)
                                         .set('system:time_start', img.get('system:time_start'))});
  }
  if(VI == "TCG"){
    var ltCollection = annualSRcollection.map(TCG)                                                   // map the function over every image in the collection - returns a 1-band annual image collection of the spectral index
                                         .map(function(img) {return img.multiply(distDir)           // ...multiply the segmentation index by the distDir to ensure that vegetation loss is associated with a positive spectral delta
                                         .set('system:time_start', img.get('system:time_start'))}); // ...set the output system:time_start metadata to the input image time_start otherwise it is null
  }
  if(VI == "TCW"){
    var ltCollection = annualSRcollection.map(TCW)                                                   // map the function over every image in the collection - returns a 1-band annual image collection of the spectral index
                                         .map(function(img) {return img.multiply(distDir)           // ...multiply the segmentation index by the distDir to ensure that vegetation loss is associated with a positive spectral delta
                                         .set('system:time_start', img.get('system:time_start'))}); // ...set the output system:time_start metadata to the input image time_start otherwise it is null
  }
  if(VI == "TCA"){
    var ltCollection = annualSRcollection.map(TCA)                                                   // map the function over every image in the collection - returns a 1-band annual image collection of the spectral index
                                         .map(function(img) {return img.multiply(distDir)           // ...multiply the segmentation index by the distDir to ensure that vegetation loss is associated with a positive spectral delta
                                         .set('system:time_start', img.get('system:time_start'))}); // ...set the output system:time_start metadata to the input image time_start otherwise it is null
  }
  if(VI == "B1"){
    var ltCollection = annualSRcollection.map(B1)                                                   // map the function over every image in the collection - returns a 1-band annual image collection of the spectral index
                                         .map(function(img) {return img.multiply(distDir)           // ...multiply the segmentation index by the distDir to ensure that vegetation loss is associated with a positive spectral delta
                                         .set('system:time_start', img.get('system:time_start'))}); // ...set the output system:time_start metadata to the input image time_start otherwise it is null
  }
  if(VI == "B1"){
  ...
  if(VI == "B2"){
    var ltCollection = annualSRcollection.map(B2)                                                   // map the function over every image in the collection - returns a 1-band annual image collection of the spectral index
                                         .map(function(img) {return img.multiply(distDir)           // ...multiply the segmentation index by the distDir to ensure that vegetation loss is associated with a positive spectral delta
                                         .set('system:time_start', img.get('system:time_start'))}); // ...set the output system:time_start metadata to the input image time_start otherwise it is null
  }
  if(VI == "B3"){
    var ltCollection = annualSRcollection.map(B3)                                                   // map the function over every image in the collection - returns a 1-band annual image collection of the spectral index
                                         .map(function(img) {return img.multiply(distDir)           // ...multiply the segmentation index by the distDir to ensure that vegetation loss is associated with a positive spectral delta
                                         .set('system:time_start', img.get('system:time_start'))}); // ...set the output system:time_start metadata to the input image time_start otherwise it is null
  }
  if(VI == "B4"){
    var ltCollection = annualSRcollection.map(B4)                                                   // map the function over every image in the collection - returns a 1-band annual image collection of the spectral index
                                         .map(function(img) {return img.multiply(distDir)           // ...multiply the segmentation index by the distDir to ensure that vegetation loss is associated with a positive spectral delta
                                         .set('system:time_start', img.get('system:time_start'))}); // ...set the output system:time_start metadata to the input image time_start otherwise it is null
  }
  if(VI == "B5"){
    var ltCollection = annualSRcollection.map(B5)                                                   // map the function over every image in the collection - returns a 1-band annual image collection of the spectral index
                                         .map(function(img) {return img.multiply(distDir)           // ...multiply the segmentation index by the distDir to ensure that vegetation loss is associated with a positive spectral delta
                                         .set('system:time_start', img.get('system:time_start'))}); // ...set the output system:time_start metadata to the input image time_start otherwise it is null
  }
  if(VI == "B7"){
    var ltCollection = annualSRcollection.map(B7)                                                   // map the function over every image in the collection - returns a 1-band annual image collection of the spectral index
                                         .map(function(img) {return img.multiply(distDir)           // ...multiply the segmentation index by the distDir to ensure that vegetation loss is associated with a positive spectral delta
                                         .set('system:time_start', img.get('system:time_start'))}); // ...set the output system:time_start metadata to the input image time_start otherwise it is null
  }
