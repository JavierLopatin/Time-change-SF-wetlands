...
"""def to_tiff(xarray, raster):
    ''' Use rasterio to save results into disk'''
    import rasterio
    # set output name of the GeoTIFF
    outfile = raster[:-4] + '_phen.tif'
    # load and update metadata
    with rasterio.open(raster) as src:
        profile = src.profile
        dtype = xarray.dtype
        profile.update(count=len(xarray['time']), dtype=dtype)
    # save results
